...
public class GravityActivity extends AppCompatActivity implements SensorEventListener {
    ...
    private GravityActivity.PaperView paperView;
    private Paint paint;
    private Path path;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        paint = new Paint();
        path = new Path();
        paperView = new GravityActivity.PaperView(this);
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.sensor.equals(mRotSensor) && sensorOn) {
            ...
            else if (cos_sin[2] < 0.7) {
                distance = 13;
            }
            else {
                distance = 40;
            }
            int[] duration_buzz = setImgPos_getDurationBuzz(cos_sin, distance, 10);
    ...
    public float[] getDirectionFromOrientation(float xRad, float yRad) {
        ...
        float[] cos_sin = {0, 0, Math.abs(xRad)+Math.abs(yRad)};
        ...
        Log.d("xy", twoArraytoString(new float[] {Math.abs(xRad), Math.abs(yRad)}));
        return cos_sin;
    }
